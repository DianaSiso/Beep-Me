...
@Component({
  templateUrl: './VIT.component.html',
  styleUrls: ['./VIT.component.css']
})


export class VITComponent implements OnInit {
  ...
  fetchRestStamps():void{
    this.httpClient.get<any>('http://deti-engsoft-02.ua.pt:8080/restaurant/orders/perhour').subscribe(res=>
    {
      ...
      for (var value in res.vitaminas) {  
         this.map1.set(value,res.vitaminas[value])  
      }
  ...
  fetchTest():void{ //Mudar rest_id //
    this.httpClient.get<any>('http://deti-engsoft-02.ua.pt:8080//orders/restaurant?rest_id=10').subscribe(res=>
    {
